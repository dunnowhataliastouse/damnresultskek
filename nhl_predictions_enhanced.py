...
class LineChemistryAnalyzer:
    ...
    def get_team_line_strength(self, team_abbrev: str, before_date: str) -> float:
        ...
        top_combos = sorted(combos.items(), key=lambda x: x[1]["goals"], reverse=True)[:5]
    ...
    def get_defense_rating(self, team_abbrev: str, before_date: str) -> float:
        ...
        if team_abbrev not in self.team_defense_ratings:
            self.analyze_defensive_pairings(team_abbrev, before_date)
        return self.team_defense_ratings.get(team_abbrev, 0.5)
...
class EnhancedMatchPredictor:
    ...
    def __init__(self, db_path: str = "nhl_stats.db"):
        self.db_path = db_path
        self.line_analyzer = LineChemistryAnalyzer(db_path)
...
class EnhancedPlayerPredictor:
    ...
    def get_player_historical_stats(self, player_id: int, before_date: str,
        ...
        stats = {
            ...
            "team": player["team_abbrev"] if player else "",
            "games_analyzed": len(games),
            "avg_shots": statistics.mean(shots) if shots else 1.5,
    ...
    def get_game_context(self, player_team: str, opponent: str, game_date: str,
        ...
        pace_factor = ((team_shot_pace / league_avg_team_shots) + (opp_allows / league_avg_team_shots)) / 2
    ...
    def calculate_probability_over_threshold(self, expected: float, std: float,
        ...
        """
        Calculate probability of exceeding a threshold using normal approximation.
        For discrete counts, we use continuity correction.
        """
        if std <= 0:
            return 1.0 if expected > threshold else 0.0

        # Continuity correction for discrete values
        adjusted_threshold = threshold - 0.5

        # Z-score calculation
        z = (adjusted_threshold - expected) / std

        # Use error function approximation for normal CDF
        # P(X > threshold) = 1 - P(X <= threshold) = 1 - Phi(z)
    ...
    def predict_player_game(self, player_id: int, opponent: str, game_date: str,
        ...
        # Weight: 50% recent form, 30% season average, 20% position baseline
        if player_stats["games_analyzed"] >= 5:
            expected_shots = (
                0.50 * player_stats["recent_5_shots"] +
                0.30 * player_stats["avg_shots"] +
                0.20 * baseline["shots"]
            )
        ...
        if player_stats["games_analyzed"] >= 5:
            expected_blocked = (
                0.50 * player_stats["recent_5_blocked"] +
                0.30 * player_stats["avg_blocked"] +
        ...
        shots_confidence = round(games_factor * 0.6 + consistency_factor * 0.4, 3)
    ...
    def predict_team_players(self, team_abbrev: str, opponent: str, game_date: str,
                             is_home: bool, limit: int = 20) -> List[Dict[str, Any]]:
        """
...
def run_player_prediction_demo():
    ...
    predictor = EnhancedPlayerPredictor()
