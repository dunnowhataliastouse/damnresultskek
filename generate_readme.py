...
def generate_predictions_readme(game_date: str, output_path: str = "predictions/README.md",
    ...
    # Add 10-day accuracy summary section
    lines.append("")
    lines.append("---")
    lines.append("")
    lines.append("## 10-Day Prediction Accuracy Summary")
    lines.append("")

    try:
        accuracy_results = predictor.compare_predictions_vs_results(lookback_days=10)
        if "error" not in accuracy_results:
            overall = accuracy_results["overall_accuracy"]
            lines.append(f"*Period: {accuracy_results['period']}*")
            lines.append("")
            lines.append("### Overall Performance")
            lines.append("")
            lines.append(f"| Metric | Value |")
            lines.append(f"|--------|-------|")
            lines.append(f"| Total Predictions | {accuracy_results['total_predictions']} |")
            lines.append(f"| 2+ SOG Accuracy | {overall['accuracy_2plus']}% |")
            lines.append(f"| Actual 2+ SOG Rate | {overall['actual_2plus_rate']}% |")
            lines.append(f"| Mean Absolute Error | {overall['mae']} SOG |")
            lines.append("")

            # Top 15 accurate players by team
            if accuracy_results.get("top_15_by_team"):
                lines.append("### Top 15 Most Accurate Players (Sorted by Team)")
                lines.append("")
                lines.append("| Team | Player | Pos | Games | Accuracy | Avg Exp | Avg Act |")
                lines.append("|------|--------|-----|-------|----------|---------|---------|")

                for team, players in sorted(accuracy_results["top_15_by_team"].items()):
                    for p in players:
                        lines.append(f"| {p['team']} | {p['player_name'][:22]} | {p['position']} | {p['predictions']} | {p['accuracy_2plus']}% | {p['avg_expected']:.2f} | {p['avg_actual']:.2f} |")
                lines.append("")
        else:
            lines.append("*No historical predictions available for accuracy comparison.*")
            lines.append("")
    except Exception as e:
        lines.append(f"*Accuracy summary unavailable: {str(e)}*")
        lines.append("")
