# NHL Shots on Goal Predictions (Ladbrokes Filtered)
## 2025-12-13

*Generated: 2025-12-14 19:49:22*

*Only showing players with Ladbrokes SOG markets*

*Injury data loaded: 129 players on injury report*

---

### VGK @ CBJ
*Ladbrokes: 15 players with SOG markets*

| Player | Pos | Team | Exp SOG | Hist Avg | Variance | P(2+) | P(3+) | LB Line | Over | Under | Status |
|--------|-----|------|---------|----------|----------|-------|-------|---------|------|-------|--------|
| Jack Eichel | C | VGK | 3.27 | 3.53 | Variable | 87% | 69% | 3.5 | - | - |  |
| Tomas Hertl | C | VGK | 2.57 | 2.73 | Volatile | 71% | 51% | 2.5 | - | - |  |
| Adam Fantilli | C | CBJ | 2.13 | 2.67 | Volatile | 66% | 40% | 2.5 | - | - |  |
| Cole Sillinger | C | CBJ | 1.92 | 2.20 | Volatile | 59% | 37% | 1.5 | - | - |  |
| Dmitri Voronkov | L | CBJ | 1.56 | 1.60 | Volatile | 52% | 21% | 1.5 | - | - |  |
| Mark Stone | R | VGK | 2.78 | 2.67 | Volatile | 75% | 56% | 1.5 | - | - |  |
| Pavel Dorofeyev | R | VGK | 2.56 | 2.40 | Volatile | 74% | 52% | 2.5 | - | - |  |
| Kirill Marchenko | R | CBJ | 2.33 | 2.73 | Volatile | 69% | 46% | 2.5 | - | - |  |
| Mitch Marner | R | VGK | 1.92 | 1.73 | Volatile | 61% | 35% | 1.5 | - | - |  |
| Braeden Bowman | R | VGK | 1.92 | 1.80 | Volatile | 62% | 33% | 1.5 | - | - |  |
| Zach Werenski | D | CBJ | 2.72 | 3.80 | Volatile | 75% | 55% | 2.5 | - | - |  |
| Shea Theodore | D | VGK | 2.72 | 2.73 | Volatile | 76% | 55% | 1.5 | - | - |  |
| Noah Hanifin | D | VGK | 2.54 | 2.40 | Variable | 81% | 51% | 1.5 | - | - |  |

### DET @ CHI
*Ladbrokes: 10 players with SOG markets*

| Player | Pos | Team | Exp SOG | Hist Avg | Variance | P(2+) | P(3+) | LB Line | Over | Under | Status |
|--------|-----|------|---------|----------|----------|-------|-------|---------|------|-------|--------|
| Dylan Larkin | C | DET | 2.68 | 3.00 | Volatile | 74% | 54% | 2.5 | - | - |  |
| Ryan Greene | C | CHI | 1.58 | 1.93 | Volatile | 52% | 26% | 1.5 | - | - |  |
| Lucas Raymond | L | DET | 2.13 | 2.47 | Volatile | 64% | 41% | 2.5 | - | - |  |
| Tyler Bertuzzi | L | CHI | 2.02 | 2.40 | Volatile | 66% | 36% | 2.5 | - | - |  |
| Alex DeBrincat | R | DET | 2.68 | 3.00 | Variable | 82% | 55% | 2.5 | - | - |  |
| Patrick Kane | R | DET | 2.68 | 2.93 | Volatile | 76% | 54% | 2.5 | - | - |  |
| Ilya Mikheyev | R | CHI | 1.58 | 1.80 | Volatile | 53% | 22% | 1.5 | - | - |  |
| Moritz Seider | D | DET | 2.24 | 2.47 | Variable | 76% | 40% | 2.5 | - | - |  |

### NSH @ COL
*Ladbrokes: 15 players with SOG markets*

| Player | Pos | Team | Exp SOG | Hist Avg | Variance | P(2+) | P(3+) | LB Line | Over | Under | Status |
|--------|-----|------|---------|----------|----------|-------|-------|---------|------|-------|--------|
| Nathan MacKinnon | C | COL | 5.08 | 5.07 | Stable | 97% | 91% | 4.5 | - | - |  |
| Ross Colton | C | COL | 2.40 | 2.53 | Volatile | 69% | 48% | 1.5 | - | - |  |
| Martin Necas | C | COL | 2.18 | 2.00 | Variable | 74% | 38% | 2.5 | - | - |  |
| Steven Stamkos | C | NSH | 2.12 | 2.33 | Volatile | 60% | 44% | 1.5 | - | - |  |
| Brock Nelson | C | COL | 1.97 | 1.87 | Volatile | 65% | 33% | 1.5 | - | - |  |
| Gabriel Landeskog | L | COL | 2.81 | 2.67 | Volatile | 76% | 56% | 2.5 | - | - |  |
| Artturi Lehkonen | L | COL | 2.63 | 2.47 | Variable | 80% | 54% | 2.5 | - | - |  |
| Filip Forsberg | L | NSH | 2.47 | 3.20 | Volatile | 70% | 49% | 2.5 | - | - |  |
| Valeri Nichushkin | R | COL | 2.26 | 2.13 | Volatile | 68% | 44% | 1.5 | - | - |  |
| Luke Evangelista | R | NSH | 1.91 | 2.53 | Volatile | 60% | 36% | 1.5 | - | - |  |
| Cale Makar | D | COL | 3.23 | 3.40 | Volatile | 77% | 62% | 2.5 | - | - |  |
| Devon Toews | D | COL | 2.24 | 2.07 | Volatile | 69% | 43% | 1.5 | - | - |  |
| Brent Burns | D | COL | 2.24 | 2.20 | Variable | 74% | 41% | 1.5 | - | - |  |
| Roman Josi | D | NSH | 2.03 | 2.73 | Volatile | 64% | 37% | 2.5 | - | - |  |

### FLA @ DAL
*Ladbrokes: 14 players with SOG markets*

| Player | Pos | Team | Exp SOG | Hist Avg | Variance | P(2+) | P(3+) | LB Line | Over | Under | Status |
|--------|-----|------|---------|----------|----------|-------|-------|---------|------|-------|--------|
| Carter Verhaeghe | C | FLA | 2.32 | 2.60 | Variable | 74% | 45% | 2.5 | - | - |  |
| Anton Lundell | C | FLA | 2.22 | 2.60 | Volatile | 67% | 43% | 1.5 | - | - |  |
| Sam Bennett | C | FLA | 2.18 | 2.53 | Volatile | 65% | 43% | 2.5 | - | - |  |
| Wyatt Johnston | C | DAL | 1.85 | 2.20 | Volatile | 58% | 36% | 2.5 | - | - |  |
| Sam Reinhart | C | FLA | 1.62 | 1.93 | Lower* | 55% | 18% | 2.5 | - | - |  |
| Evan Rodrigues | C | FLA | 1.55 | 1.80 | Volatile | 51% | 27% | 1.5 | - | - |  |
| Jason Robertson | L | DAL | 3.30 | 4.00 | Lower* | 86% | 68% | 3.5 | - | - |  |
| Brad Marchand | L | FLA | 2.34 | 2.67 | Volatile | 64% | 47% | 2.5 | - | - |  |
| Mikko Rantanen | R | DAL | 2.26 | 2.53 | Variable | 75% | 42% | 2.5 | - | - |  |
| Mackie Samoskevich | R | FLA | 2.08 | 2.33 | Volatile | 64% | 40% | 1.5 | - | - |  |
| Miro Heiskanen | D | DAL | 1.94 | 2.20 | Variable | 66% | 30% | 1.5 | - | - |  |
| Seth Jones | D | FLA | 1.84 | 2.20 | Volatile | 58% | 35% | 1.5 | - | - |  |

### CGY @ LAK
*Ladbrokes: 13 players with SOG markets*

| Player | Pos | Team | Exp SOG | Hist Avg | Variance | P(2+) | P(3+) | LB Line | Over | Under | Status |
|--------|-----|------|---------|----------|----------|-------|-------|---------|------|-------|--------|
| Nazem Kadri | C | CGY | 2.54 | 3.53 | Volatile | 70% | 51% | 2.5 | - | - |  |
| Anze Kopitar | C | LAK | 1.98 | 1.80 | Variable | 67% | 31% | 1.5 | - | - |  |
| Kevin Fiala | L | LAK | 3.07 | 2.93 | Volatile | 77% | 60% | 2.5 | - | - |  |
| Blake Coleman | L | CGY | 1.95 | 2.07 | Volatile | 62% | 35% | 1.5 | - | - |  |
| Trevor Moore | L | LAK | 1.70 | 1.67 | Volatile | 57% | 24% | 1.5 | - | - |  |
| Adrian Kempe | R | LAK | 2.82 | 3.00 | Variable | 85% | 60% | 2.5 | - | - |  |
| Matt Coronato | R | CGY | 2.47 | 3.27 | Volatile | 67% | 49% | 2.5 | - | - |  |
| Quinton Byfield | R | LAK | 2.16 | 2.20 | Volatile | 64% | 43% | 1.5 | - | - |  |
| Alex Laferriere | R | LAK | 2.16 | 2.13 | Variable | 70% | 39% | 2.5 | - | - |  |
| Rasmus Andersson | D | CGY | 2.31 | 2.73 | Volatile | 65% | 46% | 2.5 | - | - |  |
| MacKenzie Weegar | D | CGY | 2.00 | 2.67 | Volatile | 61% | 39% | 1.5 | - | - |  |
| Brandt Clarke | D | LAK | 1.91 | 1.93 | Variable | 65% | 28% | 1.5 | - | - |  |

---

## Top 15 Shooters (Sorted by Team, then Confidence)

| Rank | Player | Team | Matchup | Exp SOG | Hist Avg | Variance | P(2+) | P(3+) | LB Line | Status |
|------|--------|------|---------|---------|----------|----------|-------|-------|---------|--------|
| 1 | Zach Werenski | CBJ | vs VGK | 2.72 | 3.80 | Volatile | 75% | 55% | 2.5 |  |
| 2 | Nathan MacKinnon | COL | vs NSH | 5.08 | 5.07 | Stable | 97% | 91% | 4.5 |  |
| 3 | Artturi Lehkonen | COL | vs NSH | 2.63 | 2.47 | Variable | 80% | 54% | 2.5 |  |
| 4 | Cale Makar | COL | vs NSH | 3.23 | 3.40 | Volatile | 77% | 62% | 2.5 |  |
| 5 | Gabriel Landeskog | COL | vs NSH | 2.81 | 2.67 | Volatile | 76% | 56% | 2.5 |  |
| 6 | Jason Robertson | DAL | vs FLA | 3.30 | 4.00 | Lower* | 86% | 68% | 3.5 |  |
| 7 | Alex DeBrincat | DET | @ CHI | 2.68 | 3.00 | Variable | 82% | 55% | 2.5 |  |
| 8 | Patrick Kane | DET | @ CHI | 2.68 | 2.93 | Volatile | 76% | 54% | 2.5 |  |
| 9 | Dylan Larkin | DET | @ CHI | 2.68 | 3.00 | Volatile | 74% | 54% | 2.5 |  |
| 10 | Adrian Kempe | LAK | vs CGY | 2.82 | 3.00 | Variable | 85% | 60% | 2.5 |  |
| 11 | Kevin Fiala | LAK | vs CGY | 3.07 | 2.93 | Volatile | 77% | 60% | 2.5 |  |
| 12 | Jack Eichel | VGK | @ CBJ | 3.27 | 3.53 | Variable | 87% | 69% | 3.5 |  |
| 13 | Shea Theodore | VGK | @ CBJ | 2.72 | 2.73 | Volatile | 76% | 55% | 1.5 |  |
| 14 | Mark Stone | VGK | @ CBJ | 2.78 | 2.67 | Volatile | 75% | 56% | 1.5 |  |
| 15 | Tomas Hertl | VGK | @ CBJ | 2.57 | 2.73 | Volatile | 71% | 51% | 2.5 |  |

---

## High Confidence Picks (P(2+) > 70%) - Sorted by Team, then Confidence

| Player | Team | Exp SOG | Hist Avg | Variance | P(2+) | P(3+) | LB Line | Status |
|--------|------|---------|----------|----------|-------|-------|---------|--------|
| Zach Werenski | CBJ | 2.72 | 3.80 | Volatile | 75% | 55% | 2.5 |  |
| Nathan MacKinnon | COL | 5.08 | 5.07 | Stable | 97% | 91% | 4.5 |  |
| Artturi Lehkonen | COL | 2.63 | 2.47 | Variable | 80% | 54% | 2.5 |  |
| Cale Makar | COL | 3.23 | 3.40 | Volatile | 77% | 62% | 2.5 |  |
| Gabriel Landeskog | COL | 2.81 | 2.67 | Volatile | 76% | 56% | 2.5 |  |
| Brent Burns | COL | 2.24 | 2.20 | Variable | 74% | 41% | 1.5 |  |
| Martin Necas | COL | 2.18 | 2.00 | Variable | 74% | 38% | 2.5 |  |
| Jason Robertson | DAL | 3.30 | 4.00 | Lower* | 86% | 68% | 3.5 |  |
| Mikko Rantanen | DAL | 2.26 | 2.53 | Variable | 75% | 42% | 2.5 |  |
| Alex DeBrincat | DET | 2.68 | 3.00 | Variable | 82% | 55% | 2.5 |  |
| Patrick Kane | DET | 2.68 | 2.93 | Volatile | 76% | 54% | 2.5 |  |
| Moritz Seider | DET | 2.24 | 2.47 | Variable | 76% | 40% | 2.5 |  |
| Dylan Larkin | DET | 2.68 | 3.00 | Volatile | 74% | 54% | 2.5 |  |
| Carter Verhaeghe | FLA | 2.32 | 2.60 | Variable | 74% | 45% | 2.5 |  |
| Adrian Kempe | LAK | 2.82 | 3.00 | Variable | 85% | 60% | 2.5 |  |
| Kevin Fiala | LAK | 3.07 | 2.93 | Volatile | 77% | 60% | 2.5 |  |
| Alex Laferriere | LAK | 2.16 | 2.13 | Variable | 70% | 39% | 2.5 |  |
| Filip Forsberg | NSH | 2.47 | 3.20 | Volatile | 70% | 49% | 2.5 |  |
| Jack Eichel | VGK | 3.27 | 3.53 | Variable | 87% | 69% | 3.5 |  |
| Noah Hanifin | VGK | 2.54 | 2.40 | Variable | 81% | 51% | 1.5 |  |
| Shea Theodore | VGK | 2.72 | 2.73 | Volatile | 76% | 55% | 1.5 |  |
| Mark Stone | VGK | 2.78 | 2.67 | Volatile | 75% | 56% | 1.5 |  |
| Pavel Dorofeyev | VGK | 2.56 | 2.40 | Volatile | 74% | 52% | 2.5 |  |
| Tomas Hertl | VGK | 2.57 | 2.73 | Volatile | 71% | 51% | 2.5 |  |

---

## Summary

| Metric | Value |
|--------|-------|
| Games with Ladbrokes data | 5 |
| Total Ladbrokes players | 67 |
| Matched to predictions | 59 |
| High confidence picks (P(2+) > 70%) | 24 |

---

## Value Analysis

*Comparing our probabilities to Ladbrokes implied odds*

*No significant value picks found (edge > 5%)*

---

## Legend

### Columns
- **Exp SOG**: Expected Shots on Goal (prediction)
- **Hist Avg**: Historical average SOG over recent games
- **Variance**: How prediction compares to historical average
- **P(2+)**: Probability of 2 or more shots
- **P(3+)**: Probability of 3 or more shots
- **LB Line**: Ladbrokes line (e.g., 1.5 = Over/Under 1.5 shots)
- **Over/Under**: Ladbrokes decimal odds
- **Edge**: Difference between our probability and implied odds probability
- **Status**: Injury status (blank = healthy)

### Variance Indicators
| Indicator | Meaning |
|-----------|---------|
| **Stable** | Prediction close to historical average (consistent player) |
| **Slight+** | Prediction 10-20% above historical average |
| **Slight-** | Prediction 10-20% below historical average |
| **Higher** | Prediction >20% above historical average |
| **Lower** | Prediction >20% below historical average |
| **Variable** | Player has moderate game-to-game variance |
| **Higher*** | Above average but inconsistent (use caution) |
| **Lower*** | Below average but inconsistent (use caution) |
| **Volatile** | High variance player - unpredictable |

### Injury Status
| Status | Meaning |
|--------|---------|
| **OUT** | Confirmed out, not playing |
| **IR** | Injured Reserve |
| **LTIR** | Long-Term Injured Reserve |
| **DTD** | Day-to-Day (may or may not play) |
| **SUSPENDED** | Suspended from play |

---

*Predictions generated using EnhancedPlayerPredictor with position-specific TOI filtering*
- Forwards (C, L, R): 14+ min average TOI
- Defensemen (D): 16+ min average TOI
- Injury data from Daily Faceoff (when available)